#################################################################################
#
#  Project Name  : Reliance Stock Price Forecasting (LSTM)
#  Description   : Uses an LSTM (Long Short-Term Memory) neural network to
#                  forecast the next-day closing price of Reliance stock.
#                  The pipeline reads historical Close prices from a CSV file,
#                  scales them with Min-Max scaling, converts them into
#                  time-series sequences (previous 10 days -> next day),
#                  trains a stacked LSTM model, evaluates it (MAE / MSE / RMSE),
#                  and predicts the next-day price. Every step prints detailed
#                  intermediate calculations for learning purposes.
#  Date          : 25-Sep-2026
...
# Input :         title (string)
# Description :   Prints a formatted section header to visually separate the
#                 different steps of the pipeline in the console output
# Date :          25-Sep-2026
# Author :        Shubham Shitole
#
#################################################################################
...
# Input :         title (string), dataframe (pandas DataFrame), rows (int)
# Description :   Prints a section header followed by the first N rows of the
#                 given DataFrame for quick inspection
# Date :          25-Sep-2026
# Author :        Shubham Shitole
#
#################################################################################
...
Print_Header("RELIANCE STOCK FORECASTING USING LSTM")
...
#################################################################################
#
# Function Name : Create_Sequences
# Input :         dataset (scaled numpy array), time_steps (int)
# Description :   Converts a 1D time-series into supervised learning format by
#                 sliding a window of 'time_steps' days to build input features
#                 (X) and the next-day value as the target (y)
# Return Value :  X (numpy array), y (numpy array)
# Date :          25-Sep-2026
# Author :        Shubham Shitole
#
#################################################################################
...
# Input :         z (float)
# Description :   Applies the sigmoid activation: 1 / (1 + e^-z)
# Return Value :  Sigmoid of input
# Date :          25-Sep-2026
# Author :        Shubham Shitole
#
#################################################################################
...
Print_Header("STEP 14 : MODEL EVALUATION WITH CALCULATIONS")
...
plt.legend()
plt.grid(True)
plt.savefig("reliance_actual_vs_predicted.png")
plt.show()

print("Graph saved as reliance_actual_vs_predicted.png")
...
plt.legend()
plt.grid(True)
plt.savefig("reliance_training_loss.png")
plt.show()

print("Graph saved as reliance_training_loss.png")
